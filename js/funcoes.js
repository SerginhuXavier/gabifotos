...
function isPhoneClick(telefone) {
    ...
    /*
     //    var regex = new RegExp('^(?:(?([0-9]{2}))?[-. ]?)?([0-9]{4,5})[-. ]?([0-9]{4})$');
     var regex = new RegExp('^.(\d{2}.)(\d{4}).-(\d{4,5})$');
     
     if (regex.test($("#" + campo).val()) === 1) {
     return true;
     } else {
     return false;
     }
...
function isPhone(num, campo) {
    ...
    if (tecla === 8) {
        ...
        $("#" + campo).val(num + '-')
        return true;
    } else if ((tecla > 47 && tecla < 58) || (tecla >= 96 && tecla <= 105) || tecla === 109 || tecla === 189) {
        if (num.length >= 14) {
            return false;
        } else {
            return true;
