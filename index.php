<!DOCTYPE html>
<html lang="pt">
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <!-- Meta, title, CSS, favicons, etc. -->
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <title>Gabi Fotos | Sistema</title>

    <!-- Bootstrap -->
    <link href="tema/gentelella/vendors/bootstrap/dist/css/bootstrap.min.css" rel="stylesheet">
    <!-- Font Awesome -->
    <link href="tema/gentelella/vendors/font-awesome/css/font-awesome.min.css" rel="stylesheet">
    <!-- NProgress -->
    <link href="tema/gentelella/vendors/nprogress/nprogress.css" rel="stylesheet">
    <!-- Animate.css -->
    <link href="tema/gentelella/vendors/animate.css/animate.min.css" rel="stylesheet">

    <!-- Custom Theme Style -->
    <link href="tema/gentelella/build/css/custom.min.css" rel="stylesheet">
  </head>

  <body class="login">
    <div>
      <a class="hiddenanchor" id="signup"></a>
      <a class="hiddenanchor" id="signin"></a>

      <div class="login_wrapper">
        <div class="animate form login_form">
          <section class="login_content">
            <form>
              <h1>Login</h1>
              <div>
                <input type="text" class="form-control" placeholder="Usuário" required="" />
              </div>
              <div>
                <input type="password" class="form-control" placeholder="Senha" required="" />
              </div>
              <div>
                <a class="btn btn-default submit" href="principal.php">Logar</a>
              </div>

              <div class="clearfix"></div>

              <div class="separator">
                <p class="change_link">Esqueceu sua senha?
                  <a href="#signup" class="to_lembrarSenha"> Clique aqui </a>
                </p>

                <div class="clearfix"></div>
                <br />

                <div>
                  <h1><i class="fa fa-camera"></i> Gabi Fotos</h1>
                  <p>
					©<?php echo date("Y"); ?> Todos os direitos reservados
					paulosergioxavier.com.br
				  </p>
                </div>
              </div>
            </form>
          </section>
        </div>

        <div id="lembrarSenha" class="animate form registration_form">
          <section class="login_content">
            <form>
              <h1>Lembrar Senha</h1>
              <div>
                <input type="email" class="form-control" placeholder="Email" required="" />
              </div>
              <div>
                <a class="btn btn-default submit" href="principal.php">Enviar</a>
              </div>

              <div class="clearfix"></div>

              <div class="separator">
                
                <div>
                  <h1><i class="fa fa-camera"></i> Gabi Fotos</h1>
                  <p>
					©<?php echo date("Y"); ?> Todos os direitos reservados
					paulosergioxavier.com.br
				  </p>
                </div>
              </div>
            </form>
          </section>
        </div>
      </div>
    </div>
  </body>
</html>
